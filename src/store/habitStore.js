  ...
  persist(
    (set, get) => ({
      ...
      initUserData: async (userId) => {
        ...
        if (raw) {
          try { applyData(JSON.parse(raw)); hasLocalData = true; } catch (_) {}
        }
        ...
        try {
          ...
          if (!error && data?.data) {
            ...
            // Brand new user — preload habits with empty completions (blank slate)
            const habits = JSON.parse(JSON.stringify(DEFAULT_HABITS));
            set({ habits, completions: {}, mentalState: { mood: {}, motivation: {} } });
            // Save their starter habits to Supabase immediately
            get().saveUserData();
          }
        } catch (err) {
          console.warn('Supabase fetch failed, using local cache:', err);
          if (!hasLocalData) {
            // Offline + no cache — preload habits, blank completions
            const habits = JSON.parse(JSON.stringify(DEFAULT_HABITS));
            set({ habits, completions: {}, mentalState: { mood: {}, motivation: {} } });
          }
        }
      ...
      saveUserData: async () => {
        ...
        // Instant local write
        try {
          localStorage.setItem(`habitData_${state.currentUserId}`, JSON.stringify(toSave));
        } catch (e) {
          if (e?.name === 'QuotaExceededError') console.warn('localStorage full');
        }

        // Background Supabase sync (upsert = insert or update)
        try {
          const { supabase } = await import('../lib/supabase');
          await supabase
            .from('habit_data')
          ...
          console.warn('Supabase sync failed (data is safe in localStorage):', err);
        }
      },
