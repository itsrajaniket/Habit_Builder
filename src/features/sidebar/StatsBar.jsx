...
export default function StatsBar() {
  ...
  let total = 0;
  habits.forEach(h => { for (let d = 1; d <= dim; d++) { if (completions[h.id]?.[fmt(year, month + 1, d)]) total++; } });
  const possible   = habits.length * dim;
  const pct        = possible > 0 ? Math.round((total / possible) * 100) : 0;
  const bestStreak = Math.max(0, ...habits.map(h => calcBestStreak(h.id, completions, freezeUsedDates)));

  return (
    <div className="card rounded-2xl p-4">
      <p className="text-[10px] font-bold uppercase tracking-widest t3 mb-3">📊 Dashboard</p>
      <div className="grid grid-cols-2 gap-2 mb-4">
        {[
          ...
          <div key={l} className="card-hi rounded-xl px-3 py-2.5">
            <div className="text-2xl font-black leading-none" style={{ color: c }}>{v}</div>
            <div className="text-[10px] t3 mt-1 font-semibold uppercase tracking-wide">{l}</div>
          </div>
        ))}
      </div>
      <div>
        <div className="flex justify-between items-center mb-1.5">
          ...
          <span className="text-sm font-bold" style={{ color:'var(--green)' }}>{pct}%</span>
        </div>
