...
export default function XPLevelCard() {
  ...
  for (let d = 1; d <= dim; d++) {
    ...
    doneList.forEach((h) => {
      totalXP += XP_HABIT;
      if (calcStreak(h.id, completions, freezeUsedDates) >= 3)
        totalXP += XP_STREAK;
    });
  ...
  return (
    <div style={{ padding: "14px 16px" }}>
      ...
      >
        <span style={{ fontSize: 26, flexShrink: 0 }}>{tier.icon}</span>
      ...
      >
        {[
          { value: `+${todayXP}`, label: "Today's XP", color: "var(--green)" },
          { value: `${level}/${MAX_LEVEL}`, label: "Level", color: tier.color },
      ...
      >
        ...
        {[
          ...
          [`+${XP_STREAK} XP`, "streak ≥ 3 days bonus"],
          [`+${XP_PERFECT} XP`, "perfect day bonus"],
          [`${XP_PER_LEVEL} XP`, "= 1 level up"],
        ].map(([bold, rest]) => (
          <div key={bold} style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
            <span
              style={{
                fontWeight: 700,
                color: tier.color,
        ...
        <div style={{ marginTop: 5, color: "var(--text-3)", fontSize: 10 }}>
