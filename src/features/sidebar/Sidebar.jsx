...
function StreaksSection() {
  ...
  return (
    <Collapsible title="🔥 Current Streaks" defaultOpen storageKey="streaksBody">
      {items.length === 0 ? (
        <p className="text-xs t3 py-1">Start completing habits to build streaks!</p>
      ) : (
            ...
            return (
              <div key={h.id} className="flex items-center justify-between py-2"
                ...
                <div className="flex items-center gap-2 min-w-0">
                  <div className="w-0.5 h-4 rounded-full shrink-0" style={{ background: cat.accent }} />
                  <span className="text-[13px] t1 truncate">{h.emoji} {h.name}</span>
                </div>
                <span className="text-xs font-bold ml-2 shrink-0" style={{ color: 'var(--orange)' }}>
                  {s}d 🔥
                </span>
              </div>
            );
...
// Share Progress now lives in Analytics, so sidebar is leaner
