...
export default function HabitAnalysis({ onRemove }) {
  ...
  const items = visibleHabits.map(h => {
    let done = 0;
    for (let d = 1; d <= dim; d++) if (completions[h.id]?.[fmt(year, month+1, d)]) done++;
    return { h, done, goal: dim, streak: calcStreak(h.id, completions, freezeUsedDates) };
  ...
  const handleBulkDelete = () => {
    ...
    if (!window.confirm(`Delete ${selected.size} habit${selected.size > 1 ? 's' : ''}? This cannot be undone.`)) return;
    selected.forEach(id => removeHabit(id));
    saveUserData();
    showToast(`🗑 ${selected.size} habit${selected.size > 1 ? 's' : ''} removed.`);
    setSelected(new Set());
    setSelectMode(false);
  };
        ...
        <>
          ...
          <button onClick={() => { setSelectMode(false); setSelected(new Set()); }}
            style={{
              ...
              border: '1px solid var(--border)', cursor: 'pointer', marginLeft: 'auto',
            }}>
            Cancel
          </button>
        </>
  ...
  return (
    <Collapsible title="📊 Habit Analysis" defaultOpen storageKey="analysisBody">
      {items.length === 0
              ...
              return (
                <div key={h.id}
                  ...
                  <div className="flex items-center gap-1.5 mb-1.5">
                    ...
                    <span className="text-[13px] font-semibold t1 flex-1 truncate">{h.emoji} {h.name}</span>
                    ...
                    {!selectMode && (
                      <button onClick={() => onRemove(h.id)}
                        ...
                        onMouseLeave={e => { e.currentTarget.style.color='rgba(239,68,68,0.4)'; e.currentTarget.style.background=''; }}>
                        🗑
                      </button>
                    )}
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[11px] t3 shrink-0 w-16 text-right">{done}/{goal} · {pct}%</span>
                  </div>
                  {streak > 0 && (
                    <div className="flex items-center gap-2 mt-1.5">
                      <div className="h-1 rounded-full"
                        style={{ width:`${Math.min(100, streak*4)}%`, maxWidth:100,
                          background: streak > 14 ? 'var(--orange)' : 'var(--green)',
                          boxShadow: streak > 7 ? `0 0 6px ${streak>14?'var(--orange)':'var(--green)'}` : 'none' }} />
                      <span className="text-[10px] font-semibold" style={{ color:'var(--orange)' }}>🔥 {streak}d</span>
                    </div>
                  )}
                </div>
              );
