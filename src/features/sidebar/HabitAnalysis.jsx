...
export default function HabitAnalysis({ onRemove }) {
  ...
  const handleBulkDelete = () => {
    ...
    if (
      !window.confirm(
        `Delete ${selected.size} habit${selected.size > 1 ? "s" : ""}? This cannot be undone.`,
      )
    )
      return;
    ...
    saveUserData();
    showToast(
      `🗑 ${selected.size} habit${selected.size > 1 ? "s" : ""} removed.`,
    );
    setSelected(new Set());
    setSelectMode(false);
  };
                ...
                <div className="flex items-center gap-1.5 mb-1.5">
                  ...
                  {!selectMode && (
                    <button
                      ...
                      className="shrink-0 flex items-center justify-center rounded text-sm"
                      style={{
