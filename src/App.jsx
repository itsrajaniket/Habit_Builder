...
function AppContent() {
  ...
  const handleRemove = (id) => {
    ...
    removeHabit(id); saveUserData(); showToast('🗑 Habit removed.');
  };

  const handleAdd = (name, emoji, category, board) => {
    addHabit(name, emoji, category, board); saveUserData(); showToast('✅ Habit added!');
  };
  ...
  const handleFreeze = () => {
    ...
    if (r === 'success')      { saveUserData(); showToast('🧊 Freeze applied!'); }
  };
    ...
    <div className="app-root min-h-screen">
      ...
      {(showToday || streakFreezes > 0) && (
        <div className="px-5 pt-4 flex flex-col gap-2">
          ...
          {/* Freeze banner */}
          {streakFreezes > 0 && (
            <div className="flex items-center justify-between px-4 py-2.5 rounded-xl slide-up"
                 style={{ background:'rgba(96,165,250,0.07)', border:'1px solid rgba(96,165,250,0.18)' }}>
              <span className="text-sm" style={{ color:'var(--text-2)' }}>
                🧊 <strong style={{ color:'var(--blue)' }}>{streakFreezes}</strong> streak freeze(s) available
              </span>
              <button onClick={handleFreeze}
                className="px-3 py-1 rounded-full text-xs font-semibold transition-all"
                style={{ background:'rgba(96,165,250,0.15)', color:'var(--blue)', border:'1px solid rgba(96,165,250,0.2)' }}
      ...
      <div className="grid gap-5 px-5 pt-5 pb-0" style={{ gridTemplateColumns:'1fr 340px' }}>
        ...
        <div className="flex flex-col gap-4 min-w-0">
          ...
          <div className="card rounded-2xl p-5 slide-up">
            ...
            <button onClick={() => setShowHabitModal(true)}
              ...
              style={{ border:'1.5px dashed rgba(52,211,153,0.35)', color:'var(--green)', background:'rgba(52,211,153,0.04)' }}
          ...
          <Collapsible title="🧠 Mental State" defaultOpen storageKey="mentalStatePanel">
            <MentalStatePanel />
          </Collapsible>

          {/* Daily Progress chart — collapsible */}
          <Collapsible title="📈 Daily Progress" defaultOpen storageKey="progressChartPanel">
            <ProgressChart />
          </Collapsible>

          {/* Habit Progress Rings — between chart and XP */}
          <Collapsible title="🎯 Habit Progress Rings" defaultOpen storageKey="progressRingsPanel">
            <ProgressRings />
          </Collapsible>

          {/* XP & Level Card — collapsible */}
          <Collapsible title="⚔️ XP & Level" defaultOpen storageKey="xpLevelPanel">
            <XPLevelCard />
          </Collapsible>
      ...
      <HabitModal isOpen={showHabitModal} onClose={() => setShowHabitModal(false)} onAdd={handleAdd} />
      <Toast />
    </div>
  );
}
